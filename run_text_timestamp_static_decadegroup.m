...
%s = RandStream('mt19937ar', 'Seed', 9876543);
...
for i = 1:numel(time_labs)
  ...
  tstamps(i) = str2num(S{1});
end
% Set timestamps to decade granularity
tstamps = floor(tstamps ./ 10) * 10;
% Standardize
tstamps = tstamps - min(tstamps);
% keep this around for decade prediction
tstamp_max = max(tstamps);
...
for i = 1:numtstamps
    ...
    %disp(size(params.Znk))
    %disp(size(params.Psi))
    %disp(size(params.Phi))
    %disp(size(params.Theta))
    %disp(size(params.W))
    %disp(max(params.Xpn.inds(:,3)));

    % Update N to be number of test observations
    N = YYtest.N;
    K = size(params.Psi_s,1);
    ...
    % Keep overwriting
    save(['./' name '_params.mat'], 'params', 'params_test');
    ...
    % Znk is all ones for a static matrix
...
Xtest = Xtest * tstamp_max;
% Predict time stamp
Xpred = tstamps_unq(maxind,:) * tstamp_max;
tstamps_unq_dec = tstamps_unq * tstamp_max;

% L1 error, E[L1] and accuracy
L1 = sum(abs(Xtest - Xpred),2) / 10; % Units of decades
%L1err = sum(L1);
L1mean = mean(L1);
...
for i = 1:MCSAMP
    ...
    L1base = sum(abs(Xtest - tstamps_unq_dec(samps,:)),2) / 10; % Units of decades
    L1baseline = L1baseline + mean(L1base);
    accuracybaseline = accuracybaseline + sum(sum(abs(Xtest - tstamps_unq_dec(samps,:)),2) < 1e-4) / N;
end

L1baseline = L1baseline / MCSAMP;
accuracybaseline = accuracybaseline / MCSAMP;

fprintf('Results (in units of decades):\n');
L1mean
L1baseline
accuracy
accuracybaseline

save(['./' name '_results.mat'], ...
  'params','L1','L1mean','accuracy','L1baseline','accuracybaseline');
