...
{
  unsigned int NN, K, X_sum, P, D;
  double *X_inds, *X_vals;
  double *lPhi, *lTheta, *Znk, *lPsi;
  double *inds, *vals, *num_nz;
  double eps;

  if(nlhs != NOUTPUTS)
    mexErrMsgTxt("Usage: see \"help sample_Xpn_meat\"");
  if(nrhs != NINPUTS)
    mexErrMsgTxt("Usage: see \"help sample_Xpn_meat\"");
  ...
  for(int ii = 0; ii < NN; ii++)
  {
    ...
    for(int k = 0; k < K; k++)
    {
      // Znk is NxK and lPsi is KxN so we index them backwards
      zeta[k] = lPhi[p + k*P] + lTheta[k] + lPsi[k + n*K];
      if(zeta[k] > zeta_max)
        zeta_max = zeta[k];
    }
    for(int k = 0; k < K; k++)
